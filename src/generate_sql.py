...
# Definir la cantidad total de registros
...
with open('output.sql', 'w') as file:
    ...
    file.write('INSERT INTO `advertising_images` (`adi_adv_id`, `adi_item`, `adi_name`, `adi_path`) VALUES\n')
    for i in range(1, total_registros + 1):
        if i == total_registros:
    ...
    for i in range(1, total_registros + 1):
        columna2_valor = (i - 1) // (total_registros // 7)
        columna2_valor = min(columna2_valor, 6)  # Asegurarse de que el valor no exceda 6
        if i == total_registros:
